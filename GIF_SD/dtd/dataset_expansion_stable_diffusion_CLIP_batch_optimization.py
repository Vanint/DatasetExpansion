...
class Custom_ImageFolder(data.Dataset): 
    def __init__(self, root, loader=default_loader, split='1', train=True):
        ...
        self.imgs, self.labels = make_dataset(filename, root, class_to_idx)
        assert (len(self.imgs) == len(self.labels))
        if len(self.imgs) == 0:
            raise(RuntimeError("Found 0 images in subfolders of: " + root + "\n"
                               "Supported image extensions are: " + ",".join(IMG_EXTENSIONS)))

        rgb_mean = (0.507, 0.487, 0.441)
        ra_params = dict(translate_const=int(224 * 0.45), img_mean=tuple([min(255, round(255 * x)) for x in rgb_mean]),)
        normalize = transforms.Normalize(mean=[0.5329876098715876, 0.474260843249454, 0.42627281899380676],
...
def main():
    ...
    # Data 
    num_classes=47  
    total_trainset = Custom_ImageFolder(args.data_dir,  train=True)    
    ...
    for batch_idx, (original_inputs, augmented_inputs, targets, index) in enumerate(train_loader):
        with torch.no_grad():
            ...
            with GIF_model.ema_scope():
                ...
                samples = DDIM_sampler.decode(z_enc, c, t_enc, unconditional_guidance_scale=args.scale, unconditional_conditioning=uc,) 
